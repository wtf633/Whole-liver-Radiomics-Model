...
def model_eval_gpu(model, datafile, val_transforms):
    data_loader = DataLoader(Dataset(data=datafile, transform=val_transforms), batch_size=40, shuffle=False,
    ...
    return all_OSoutputs, all_PFSoutputs, all_OSevents, all_OSdurations, all_PFSevents, all_PFSdurations, all_ID, all_Ageoutputs, all_feature
...
def main():
    ...
    rand_p = 0.55  # 0.35
    # data augmentation random proportion
    lr = 0.00001
    lr_decay = 0.5  #
    drop_rate = 0.25  # 0.15 model dropout rate
    n_loss = 4  # 5
    ...
    train_dir = "/home/image019/image_003/Sub-network1/Data/Train/Input_Train/"
    test_dir = "/home/image019/image_003/Sub-network1/Data/Train/Input_Val/"
    val_dir = "/home/image019/image_003/Sub-network1/Data/Valid/val_Resized/"
    ...
    val_loader = DataLoader(val_ds, batch_size=val_batch, shuffle=False, num_workers=4, collate_fn=list_data_collate,
                            pin_memory=True)
    # pin_memory=torch.cuda.is_available())

    # load the test data
    test_ds = Dataset(data=test_files, transform=val_transforms)
    test_loader = DataLoader(test_ds, batch_size=test_batch, shuffle=False, num_workers=4,
    ...
    print(example_data["OS_status"], example_data["OS_time"])
    ...
    if model_train:
        ...
        t = trange(max_epochs,
                   desc=f"AE survival -- epoch 0, avg loss: inf", leave=True)
        ...
        for epoch in t:
            ...
            with torch.no_grad():
                ...
                writer.add_scalar("train 12  CP accuracy: epoch", t_acc[1].item(), epoch)
            ...
            if (epoch + 1) % val_interval == 0:
                ...
                with torch.no_grad():
                    ...
                    for val_data in val_loader:
                        ...
                        writer.add_scalar("valid 0 AE loss: step", v_loss_img.item(),
                                          val_epoch_len * epoch + val_step)
                    ...
                    for test_data in test_loader:
                        ...
                        writer.add_scalar("test 9 label loss: step", s_loss_label.item(),
                                          test_epoch_len * epoch + test_step)
                        writer.add_scalar("test 0 AE loss: step", s_loss_img.item(),
                                          test_epoch_len * epoch + test_step)
    ...
    if model_pred:
        ...
        _, pfs_risk_train, os_train, ostime_train, pfs_train, pfstime_train, ID_train, age_train, bn_feature_train = model_eval_gpu(
            model,
            train_files,
            val_transforms)
        _, pfs_risk_val, os_val, ostime_val, pfs_val, pfstime_val, ID_val, age_val, bn_feature_val = model_eval_gpu(
            model, val_files,
            val_transforms)
        _, pfs_risk_test, os_test, ostime_test, pfs_test, pfstime_test, ID_test, age_test, bn_feature_test = model_eval_gpu(
            model,
            test_files,
            val_transforms)
    ...
    if model_run:
        model.load_state_dict(torch.load(BEST_MODEL_NAME + "_PFS.pth"))
        _, pfs_risk, bn_feature = model_run_gpu(model, test_files, val_transforms)

        model.load_state_dict(torch.load(BEST_MODEL_NAME + "_OS.pth"))
        os_risk, _, _ = model_run_gpu(model, test_files, val_transforms)
        ...
        bn_feature.to_csv(BEST_MODEL_NAME + "_feature_TCIA.csv")
