...
class MultiPlane_MultiSlice_Extract_Project(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        coronal_slices = torch.split(input_tensor, 1,
                                     dim=2)  # This gives us a tuple of length 128, where each element has shape (batch_size, channels, 1, width, height)
        Ecor = torch.cat(coronal_slices,
                         dim=2)  # lets concatenate along dimension 2 to get the desired output shape for Ecor: R^C3d×N×W×H.

        saggital_slices = torch.split(input_tensor.clone(), 1,
                                      dim=3)  # This gives us a tuple of length 128, where each element has shape (batch_size, channels, length, 1, height)
        Esag = torch.cat(saggital_slices,
                         dim=3)  # lets concatenate along dimension 3 to get the desired output shape for Ecor: R^C3d×L×N×H.

        axial_slices = torch.split(input_tensor.clone(), 1,
                                   dim=4)  # This gives us a tuple of length 128, where each element has shape (batch_size, channels, length, width, 1)
        Eax = torch.cat(axial_slices,
                        dim=4)  # lets concatenate along dimension 3 to get the desired output shape for Ecor: R^C3d×L×W×N.

        # Lets calculate S using E for X
        # after matirx multiplications, we reshape the outputs based on its plane for concatenation
        Scor = (Ecor * input_tensor).permute(0, 2, 1, 3,
                                             4).contiguous()  # Scor will now have a shape (batch_size, N, channels, width, height)
        Ssag = (Esag * input_tensor).permute(0, 3, 1, 2,
                                             4).contiguous()  # Ssag will now have a shape (batch_size, N, channels, length, height)
        Sax = (Eax * input_tensor).permute(0, 4, 1, 2,
                                           3).contiguous()  # Sax will now have a shape  (batch_size, N, channels, length, width)
...
class EmbeddingLayer(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        x = torch.cat((cls_tokens, input_tensor[:, :128, :], sep_token, input_tensor[:, 128:256, :], sep_token,
                       input_tensor[:, 256:, :], sep_token), dim=1)
...
model = M3T(in_channels=3, out_channels=1,drop_p=0.5)
print(model)
