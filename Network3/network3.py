...
def model_eval_gpu(model, datafile, val_transforms):
    data_loader = DataLoader(Dataset(data=datafile, transform=val_transforms), batch_size=4, shuffle=False,
...
class M3t(nn.Module):
    def __init__(self, drop_rate):
        ...
        self.backbone = M3T(in_channels=3, out_channels=32, drop_p=drop_rate, emb_size=768, forward_expansion=4,
                            depth=12)
        # 将ClassificationHead的linear层设置为Identity
        ...
        self.fc4 = torch.nn.Linear(768, 9)
...
def main():
    ...
    lr_decay = 0.5
    drop_rate = 0.15
    rand_p = 0.25  # 0.25 data augmentation random proportion
    n_loss = 3
    ...
    if model_pred:
        ...
        # test OS model
        model.load_state_dict(torch.load(BEST_MODEL_NAME + "_sOS.pth"))
